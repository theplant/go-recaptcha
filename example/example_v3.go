...
<style>.error{color:#ff0000;} .ack{color:#0000ff;} .grecaptcha-badge{display: none}</style><title>Recaptcha Test</title></head>
...
<p>This is a test form for the go-recaptcha package</p>`
	...
	grecaptcha.ready(function() {
		...
		grecaptcha.execute('%s', {action: 'addToCart'}).then(function(token) {
...
func processRequest(request *http.Request) (result bool) {
	...
	fmt.Println(recaptchaResponse, responseFound)
	if responseFound {
		result, err := recaptcha.Confirm("172.0.0.1", recaptchaResponse[0])
		if err != nil {
...
func homePage(writer http.ResponseWriter, request *http.Request) {
	...
	fmt.Fprint(writer, fmt.Sprintf(v3Script, recaptchaPublicKey))
